...
class HostV2Tools:
    ...
    def is_os_type_priority(cls) -> bool:
        return bool(models.GlobalSettings.get_config(models.GlobalSettings.KeyEnum.OS_TYPE_PRIORITY.value))

    @classmethod
    def get_os_type(cls, host: Dict, is_os_type_priority: bool = False) -> str:
        """根据CC的主机属性，得到可能的操作系统"""
        # bk_os_name = host.get("bk_os_name").lower() if host.get("bk_os_name") else "unknown"
    ...
    def get_cpu_arch(
        cls,
        host: Dict,
        sync_cpu_arch_by_cmdb: bool = False,
        get_default: bool = True,
        os_type: str = constants.OsType.LINUX
    ) -> Optional[str]:
        if sync_cpu_arch_by_cmdb:
