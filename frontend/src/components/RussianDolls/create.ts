...
function getDefaultComponent(params: IItem) {
  ...
  return component.name || (type === 'boolean' ? 'bk-radio'  : 'bk-input');
};
...
export function initSchema(schema: IItem, parentProp = '', level = 0): Doll[] {
  // const list: Doll[] = []
  ...
  return formatSchema(schema, parentProp, level);
